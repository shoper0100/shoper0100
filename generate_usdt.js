...
try {
    ...
    // We replace the _createUser call inside register function
    // AND add the logic to store username mapping
    ...
    // "account: _account," -> "account: _account, username: _username,"
    content = content.replace(
        'account: _account,',
        'account: _account,\n            username: _username,'
    );
    ...
    // We'll insert it before VIEW FUNCTIONS
    ...
    // 8. Update Register Function Signature
    // Replace "function register(uint _ref, address _newAcc)" with "function register(string memory username, string memory referrerUsername)"
    // And add the new logic at the start

    // Actually, replacing the text body is safer than trying to regex match the whole function.
    // However, the FiveDollarRide.sol logic uses `uint _ref` param. We need to parse strings.
    // To minimize risk, let's just REPLACE the specific lines that check requirements

    // We need to swap the whole function signature and the start logic.
    // It's safer to use regex to find the function block start.
    const regSigRegex = /function register\(uint _ref, address _newAcc\) external nonReentrant whenNotPaused \{([\s\S]*?)uint levelCost/m;

    const newRegStart = `function register(string memory username, string memory referrerUsername) external nonReentrant whenNotPaused {
