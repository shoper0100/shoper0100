...
async function main() {
    console.log("🔗 Connecting contracts on opBNB Mainnet...\n");

    const ROYALTY_ADDRESS = process.env.FIVEDOLLARRIDE_ROYALTY_MAINNET;
    const FIVEDOLLARRIDE_ADDRESS = process.env.FIVEDOLLARRIDE_MAINNET;

    if (!ROYALTY_ADDRESS) throw new Error("❌ FIVEDOLLARRIDE_ROYALTY_MAINNET not set!");
    if (!FIVEDOLLARRIDE_ADDRESS) throw new Error("❌ FIVEDOLLARRIDE_MAINNET not set!");
    ...
    console.log("\n🚀 FiveDollarRide System is LIVE on opBNB Mainnet!");

    console.log("\n📋 Important URLs:");
    console.log("Royalty Contract:", `https://opbnbscan.com/address/${ROYALTY_ADDRESS}`);
    console.log("FiveDollarRide Contract:", `https://opbnbscan.com/address/${FIVEDOLLARRIDE_ADDRESS}`);
}
