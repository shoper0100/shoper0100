...
async function main() {
    console.log("🚀 Deploying FiveDollarRide to opBNB Mainnet...\n");
    ...
    const USDT_ADDRESS = process.env.USDT_OPBNB_MAINNET;
    ...
    if (!USDT_ADDRESS) throw new Error("❌ USDT_OPBNB_MAINNET not set in .env!");
    if (!ROYALTY_ADDRESS) throw new Error("❌ FIVEDOLLARRIDE_ROYALTY_MAINNET not set in .env! Deploy Royalty first.");
    ...
    if (process.env.OPBNBSCAN_API_KEY) {
        console.log("🔍 Verifying contract on opBNBScan...");
        try {
    ...
    console.log("2. Add to .env: FIVEDOLLARRIDE_MAINNET=" + fiveDollarRideAddress);
    console.log("3. Run: node scripts/connect_contracts_mainnet.js");
}
